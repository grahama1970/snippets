...
const REQUIRED_QIDS = [
  ...
  "monocle:progress:scroll",
  "monocle:hero:section",
  "monocle:hero:eyebrow",
  "monocle:hero:heading",
  "monocle:hero:quote",
  "monocle:hero:image-frame",
  "monocle:hero:image",
  "monocle:film:section",
  "monocle:film:embed-frame",
  "monocle:film:note",
  "monocle:film:metadata",
  "monocle:rules:section",
  "monocle:rules:heading",
  "monocle:rules:item:one-side",
  "monocle:rules:item:never-waver",
  "monocle:rules:item:etiquette",
  "monocle:quotes:gallery",
  "monocle:quotes:card:inspection",
  "monocle:quotes:image:inspection",
  "monocle:quotes:card:adjustment",
  "monocle:quotes:image:adjustment",
  "monocle:quotes:card:final-warning",
  "monocle:quotes:image:final-warning",
  "monocle:verdict:section",
  "monocle:verdict:image:background",
  "monocle:verdict:quote",
  "monocle:verdict:heading",
  "monocle:modal:video",
  "monocle:footer:credit",
  "monocle:evidence:slice-note"
] as const;
...
const DESKTOP_TAB_QIDS = [
  "monocle:skip:main",
  "monocle:nav:brand-top",
  "monocle:nav:film",
  "monocle:nav:rules",
  "monocle:nav:lines",
  "monocle:nav:watch-film",
  "monocle:hero:watch-film",
  "monocle:film:external-original",
  "monocle:verdict:watch-original",
  "monocle:footer:top",
  "monocle:footer:film",
  "monocle:footer:lines"
];
...
function qidSelector(qid: string) {
  return `[data-qid="${qid}"]`;
}
...
async function currentQid(page: Page) {
  return await page.evaluate(() => {
    ...
    return active?.dataset?.qid || null;
  });
}

async function isVisible(page: Page, qid: string) {
  return await page.locator(qidSelector(qid)).first().isVisible();
}
...
async function clickAndWaitForScroll(page: Page, qid: string, targetQid: string) {
  ...
  return Boolean(box && box.y < 920 && box.y > -80);
}
...
test("Monocle Man React SPA satisfies the deterministic contract and emits evidence", async ({ browser, baseURL }) => {
  ...
  for (const viewport of viewports) {
    ...
    const heroVisible = await isVisible(page, "monocle:hero:section");
    const headingText = await page.locator(qidSelector("monocle:hero:heading")).innerText();
    ...
    const interactiveState = await Promise.all(INTERACTIVE_CONTRACT.map(async ([qid, action, title]) => {
      ...
      return {
        ...
        action: await locator.first().getAttribute("data-qs-action"),
        title: await locator.first().getAttribute("title"),
    ...
    const externalLinks = await Promise.all([
      ...
      return {
        ...
        target: await locator.getAttribute("target"),
        rel: await locator.getAttribute("rel")
      };
    }));
    ...
    if (viewport.name === "desktop") {
      ...
      const dialogClosedByEscape = !(await dialog.evaluate(element => (element as HTMLDialogElement).open));
      ...
      record(interactions, viewport.name, "desktop:escape-closes-video-modal", "keyboard-escape", "Escape closes dialog, removes iframe src, returns focus", { dialogClosedByEscape, iframeAbsentAfterEscape, focusReturnedToHero }, dialogClosedByEscape && iframeAbsentAfterEscape && focusReturnedToHero === "monocle:hero:watch-film");
      ...
      const headerDialogClosed = !(await dialog.evaluate(element => (element as HTMLDialogElement).open));
      const focusReturnedToHeader = await currentQid(page);
      record(interactions, viewport.name, "desktop:header-open-close-button", "click-header-watch-and-close", "header opens modal and close button returns focus", { headerDialogOpened, headerDialogClosed, focusReturnedToHeader }, headerDialogOpened && headerDialogClosed && focusReturnedToHeader === "monocle:nav:watch-film");
    ...
    if (viewport.name === "mobile") {
      ...
      const filmVisible = await isVisible(page, "monocle:film:section");
